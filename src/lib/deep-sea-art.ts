...
export const DEEP_SEA_ART: Record<string, Scene> = {
  'Coral reefs': {
    subject: 'a bright, sunlit reef, seen from just above the sand',
    draw: () =>
      floor(P.reef, P.sand, 60) +
      `<path d="M20 0 L34 0 L16 60 L2 60Z" fill="${P.foam}" opacity="0.10"/>` +
      `<path d="M74 0 L90 0 L78 60 L62 60Z" fill="${P.foam}" opacity="0.08"/>` +
      `<ellipse cx="30" cy="62" rx="16" ry="8" fill="${P.coral2}"/>` +
      `<path d="M72 60 q-4 -16 2 -22 q3 8 1 14 q6 -9 12 -7 q-2 8 -8 11 q8 -3 12 3 q-6 5 -13 3 q4 6 0 9Z" fill="${P.coral}"/>` +
      `<ellipse cx="92" cy="38" rx="5" ry="3" fill="${P.coral2}"/><path d="M97 38 L102 34 L102 42Z" fill="${P.coral2}"/>` +
  ...
  'The recreational diving limit': {
    subject: 'a lone diver, a taut marked line, and a surface far above',
    draw: () =>
      bg(P.shallow) +
      ...
      `<circle cx="76" cy="30" r="1.6" fill="${P.foam}" opacity="0.4"/>` +
      `<circle cx="80" cy="20" r="1" fill="${P.foam}" opacity="0.35"/>` +
      snow(41, 6, P.pale, 76),
  },
  ...
  'Giant squid': {
    ...
    draw: () =>
      ...
      Array.from({ length: 8 }, (_, i) => {
        const x = 38 + i * 6
        const sway = 10 + rnd(2, i) * 14
        const dx1 = (rnd(2, i + 9) - 0.5) * 10, dx2 = (rnd(2, i + 20) - 0.5) * 8
        const suckers = Array.from({ length: 3 }, (_, k) => {
          const t = (k + 1) / 4
          const sx = x + dx1 * t + dx2 * t * t
          const sy = 40 + (20 + sway) * t + (sway + 30 - 20 - sway) * t * t
          return `<circle cx="${sx.toFixed(1)}" cy="${sy.toFixed(1)}" r="0.8" fill="${P.bone}" opacity="0.5"/>`
        }).join('')
        return `<path d="M${x} 40 q${dx1} ${20 + sway} ${dx2} ${sway + 30}" stroke="${P.squid2}" stroke-width="2.6" fill="none" stroke-linecap="round"/>${suckers}`
      }).join('') +
      snow(2, 8),
  },

  'Vampire squid': {
    subject: 'webbed arms flared open like a cape, lit by a ring of its own small lights',
    draw: () =>
      bg(P.ink) +
      current(48, 5, P.pale, 80, 0.47) +
      `<path d="M60 20 q-24 4 -26 26 q-2 16 26 20 q28 -4 26 -20 q-2 -22 -26 -26Z" fill="${P.vamp}"/>` +
      Array.from({ length: 6 }, (_, i) => {
        const a = (i / 6) * Math.PI - Math.PI / 2
        const x1 = 60 + Math.cos(a) * 6, y1 = 34 + Math.sin(a) * 4
        const x2 = 60 + Math.cos(a) * 25, y2 = 42 + Math.sin(a) * 17
        return `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="${P.void}" stroke-width="1" opacity="0.4"/>`
      }).join('') +
      `<circle cx="52" cy="30" r="6" fill="${P.void}"/><circle cx="68" cy="30" r="6" fill="${P.void}"/>` +
      `<circle cx="50" cy="28" r="1.6" fill="${P.foam}" opacity="0.5"/><circle cx="66" cy="28" r="1.6" fill="${P.foam}" opacity="0.5"/>` +
      Array.from({ length: 10 }, (_, i) => {
        const a = (i / 10) * Math.PI * 2
        const x = 60 + Math.cos(a) * 26
        const y = 46 + Math.sin(a) * 18
        return spark(x, y, i % 2 ? P.glow2 : P.glow, 0.9)
      }).join('') +
      snow(7, 8),
  },
  ...
  Blobfish: {
    ...
    draw: () =>
      ...
      `<circle cx="48" cy="42" r="2.2" fill="${P.void}"/>` +
      snow(8, 6),
  },
  ...
  Anglerfish: {
    ...
    draw: () =>
      ...
      `<circle cx="48" cy="40" r="7" fill="${P.lure}" opacity="0.15"/>` +
  ...
  'Sperm whales hunt here': {
    ...
    draw: () =>
      ...
      `<circle cx="30" cy="22" r="1.6" fill="${P.void}" opacity="0.6"/>` +
      Array.from({ length: 6 }, (_, i) => {
        const x = 40 + i * 8
  ...
  'A hydrothermal vent': {
    ...
    draw: () =>
      ...
      `<circle cx="60" cy="24" r="1.6" fill="${P.ember}"/>` +
      Array.from({ length: 5 }, (_, i) => {
        const x = 40 + i * 10
  ...
  'Colossal squid': {
    ...
    draw: () =>
      ...
      Array.from({ length: 8 }, (_, i) => {
        const x = 34 + i * 7
        const sway = 8 + rnd(5, i) * 10
        const hooks = Array.from({ length: 2 }, (_, k) => {
          const t = (k + 1) / 3
          const hx = x + (rnd(5, i + 9) - 0.5) * 10 * t
          const hy = 46 + (16 + sway) * t
          return `<path d="M${hx.toFixed(1)} ${hy.toFixed(1)} l1.6 -1.6" stroke="${P.bone}" stroke-width="0.9" fill="none"/>`
        }).join('')
        return (
          `<path d="M${x} 46 q${(rnd(5, i + 9) - 0.5) * 10} ${16 + sway} ${(rnd(5, i + 20) - 0.5) * 6} ${sway + 26}" stroke="${P.squid}" stroke-width="3" fill="none" stroke-linecap="round"/>${hooks}` +
          `<circle cx="${x + (rnd(5, i + 9) - 0.5) * 5}" cy="${52 + sway * 0.6}" r="1.1" fill="${P.bone}"/>`
        )
      }).join('') +
      snow(5, 6),
  },
  ...
  'The deepest dive any mammal has ever made': {
    ...
    draw: () =>
      ...
      `<circle cx="26" cy="30" r="1.2" fill="${P.void}"/>` +
  ...
  'The Titanic': {
    ...
    draw: () =>
      ...
      `<rect x="30" y="36" width="6" height="10" fill="${P.abyss}"/>` +
      `<rect x="42" y="34" width="6" height="10" fill="${P.abyss}"/>` +
      `<rect x="54" y="33" width="6" height="10" fill="${P.abyss}"/>` +
      `<rect x="66" y="33" width="6" height="10" fill="${P.abyss}"/>` +
      ...
      `<rect x="44" y="14" width="4" height="20" fill="${P.iron}"/>` +
      snow(17, 8),
  },
  ...
  'Sea cucumbers, grazing': {
    ...
    draw: () =>
      ...
      Array.from({ length: 13 }, (_, i) => {
        ...
        return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(2 + rnd(23, i + 18) * 2).toFixed(1)}" fill="${P.sand}"/>`
      }).join('') +
  ...
  'The deepest-living octopus ever filmed': {
    ...
    draw: () =>
      ...
      `<circle cx="55" cy="40" r="1.6" fill="${P.void}"/><circle cx="65" cy="40" r="1.6" fill="${P.void}"/>` +
      snow(29, 6),
  },
