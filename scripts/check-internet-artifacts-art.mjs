/**
 * check-internet-artifacts-art — do Internet Artifacts' twenty-five scenes
 * survive being cut out of their own era-sky, the same way
 * check-space-elevator-art proves it for altitude?
 *
 * Same shape, same checks:
 *
 *   1. Every marker has a scene, every scene names a real marker, no two
 *      scenes claim the same subject.
 *   2. Gradients are allowed; every id is prefixed with its own scene's key
 *      and every url(#…) names an id the same scene actually defines — no
 *      <filter>, since that has no id to prefix.
 *   3. Every fill, stroke and gradient stop is one of the palette colours.
 *   4. Every subject is a CUT-OUT that reads against the real era-sky it
 *      will float on at its own year.
 *   5. Real structure inside the subject, at the width a card gets and at
 *      the width a compacted cluster falls back to.
 ...
 *
 *   node scripts/check-internet-artifacts-art.mjs [--sheet out.png]
 */
...
const { MARKERS, ZONES, END_YEAR } = await import('../src/data/internet-artifacts.ts')
const { INTERNET_ARTIFACTS_ART, P, MOOD, dominantMood, slug } = await import('../src/lib/internet-artifacts-art.ts')
...
const names = Object.keys(INTERNET_ARTIFACTS_ART)

function shot(name, w) {
  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${w}" ` +
    `viewBox="0 0 120 120">${INTERNET_ARTIFACTS_ART[name].draw()}</svg>`
...
console.log('data')
{
  const sorted = [...MARKERS].every((m, i, arr) => i === 0 || arr[i - 1].year <= m.year)
  check(sorted, 'markers are listed in increasing year order')
  check(
    MARKERS.every((m) => m.year >= ZONES[0].from && m.year <= END_YEAR),
    'every marker sits inside the zoned year range',
  )
  check(
  ...
  ok(`${MARKERS.length} markers across ${ZONES.length} zones, ${ZONES[0].from} to ${END_YEAR}`)
}
...
console.log('\ncoverage')
{
  const missing = MARKERS.filter((m) => !INTERNET_ARTIFACTS_ART[m.title]).map((m) => m.title)
  ...
  const subjects = new Set(names.map((n) => INTERNET_ARTIFACTS_ART[n].subject))
  check(subjects.size === names.length, 'no two scenes claim the same subject')
}

/* ---- 2. ids, prefixed and unique ------------------------------------------- */

console.log('\nids are safe to inline twenty-five at a time')
{
  const seen = new Map()
  let bad = 0
  for (const n of names) {
    const markup = INTERNET_ARTIFACTS_ART[n].draw()
    const prefix = slug(n)
...
console.log('\npalette')
{
  ...
  for (const n of names) {
    for (const m of INTERNET_ARTIFACTS_ART[n].draw().matchAll(/(?:fill|stroke|stop-color)="([^"]+)"/g)) {
...
/* ---- 4. a cut-out, and one you can see on its own era-sky ------------------- */

/** The real era-sky colour at a year, interpolated inside its own zone —
 *  same formula the page's `update()` uses for `.sky`. */
function skyAt(year) {
  const z = ZONES.find((z) => year >= z.from && year <= z.to) ?? ZONES[ZONES.length - 1]
  const t = Math.min(1, Math.max(0, (year - z.from) / Math.max(0.001, z.to - z.from)))
  const a = hex(z.sky[0])
  const b = hex(z.sky[1])
  return a.map((v, i) => Math.round(v + (b[i] - v) * t))
}

console.log('\nevery subject is cut out, and visible on its own era-sky')
{
  let bad = 0
  for (const n of names) {
    const marker = MARKERS.find((m) => m.title === n)
    const sky = skyAt(marker.year)
    const skyL = lum(...sky)
    const s0 = shot(n, 120)
    ...
    if (best < 2.4) notes.push(`its brightest tone is only ${best.toFixed(1)}:1 on ${marker.date} sky`)
    if (notes.length) { fail(`${n}: ${notes.join('; ')}`); bad++ }
  }
  check(bad === 0, `all ${names.length} read as cut-outs against their own era-sky`)
}
...
console.log('\nstill there at the width a compacted cluster gives')
{
  ...
  for (const n of names) {
    ...
    if (frac < FLOOR) {
      fail(`${n}: only ${(frac * 100).toFixed(0)}% of the frame survives at 64px`)
      bad++
    }
  }
...
console.log('\nmood')
{
  ...
  for (const n of names) {
    ...
    const svg = INTERNET_ARTIFACTS_ART[n].draw()
  ...
  const recomputed = Object.fromEntries(names.map((n) => [n, dominantMood(INTERNET_ARTIFACTS_ART[n].draw())]))
...
{
  let drift = 0
  for (const n of names) if (INTERNET_ARTIFACTS_ART[n].draw() !== INTERNET_ARTIFACTS_ART[n].draw()) (fail(`${n} draws differently twice`), drift++)
...
console.log('\nno emoji')
{
  ...
  for (const f of ['src/lib/internet-artifacts-art.ts', 'src/data/internet-artifacts.ts', 'src/pages/internet-artifacts.astro']) {
...
if (sheetAt >= 0) {
  ...
  const inOrder = MARKERS.map((m) => m.title).filter((t) => INTERNET_ARTIFACTS_ART[t])
  const body = inOrder
    .map((n, i) => {
      const x = (i % COLS) * CW
      ...
      return (
        ...
        `<svg x="20" y="8" width="160" height="160" viewBox="0 0 120 120">${INTERNET_ARTIFACTS_ART[n].draw()}</svg>` +
...
console.log(failures ? `\n${failures} failed.` : '\nAll internet-artifacts-art checks passed.')
